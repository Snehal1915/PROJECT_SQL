...
CREATE TABLE undergoess (
    patient INT REFERENCES patient(ssn),
    procedures INT REFERENCES procedures(code),
...
-- Inserting records into undergoess table
INSERT INTO undergoess (patient, procedures, stay, date, physician, assistingnurse)
VALUES
    (123456789, 101, 5001, '2024-01-01', 1001, 2001),
    (987654321, 102, 5002, '2024-01-02', 1002, 2002),
    (111223344, 103, 5003, '2024-01-03', 1003, 2003),
    (444555666, 104, 5004, '2024-01-04', 1004, 2004),
    (777888999, 105, 5005, '2024-01-05', 1005, 2005);
...
#2) Write a query in SQL to find the name of the nurse who are the head of their department.s
...
#5)Write a query in SQL to find the floor and block where the room number 212 belongs to.
 ...
 Select b.blockfloor, b.blockcode 
from block b 
inner join room r on b.blockfloor = r.blockfloor and b.blockcode = r.blockcode
...
INNER JOIN block b ON r.blockfloor = b.blockfloor AND r.blockcode = b.blockcode
...
INNER JOIN block b ON o.blockfloor = b.blockfloor AND o.blockcode = b.blockcode
...
JOIN procedures pr ON u.procedurecode = pr.code
WHERE p.employeeid NOT IN (
  SELECT physician 
  FROM trained_in ti
  WHERE ti.treatment IN (SELECT code FROM procedures)
);
...
FROM physician p
JOIN undergoess u ON p.employeeid = u.physician
JOIN procedures pr ON u.procedures = pr.code
WHERE p.employeeid NOT IN (
    SELECT DISTINCT t.physician
    FROM trained_in t
    WHERE t.treatment = pr.code
);
...
inner join undergoess u on p.employeeid = u.physician 
inner join procedures pr on u.procedures = pr.code inner join trained_in t 
on p.employeeid = t.physician and pr.code = t.treatment 
where u.date > t.certificationExpires;


SELECT DISTINCT p.name AS Physician_Name, p.position AS Physician_Position
FROM physician p
JOIN undergoess u ON p.employeeid = u.physician
JOIN procedures pr ON u.procedures = pr.code
JOIN trained_in t ON p.employeeid = t.physician_id AND pr.code = t.procedure_id
WHERE u.date > t.certificationexpires;
...
SELECT
    ...
    p.position AS Physician_Position,
    pr.name AS Procedure_Name,
    u.date AS Procedure_Date,
    pt.name AS Patient_Name,
    t.certificationExpires AS Certification_Expiry_Date
FROM
    physician p
JOIN
    undergoess u ON p.employeeid = u.physician
JOIN
    procedures pr ON u.procedures = pr.code
JOIN
    patient pt ON u.patient = pt.ssn
JOIN
    trained_in t ON p.employeeid = t.physician AND pr.code = t.treatment
WHERE
